...
module.exports.createMongodbCrudDelegate = function(name, plural, idProperty, collection, entityDelegate, idFilter, logger) {
	...
	function createValidationError(errors) {
		var error = new Error('Invaild entity');
		error.name = 'ValidationError';
		error.errors = errors;
		return error;
	}

	function pipeValidate(validationSet, entityObject, callback) {
		entityDelegate.validate(entityObject, validationSet, function(errors) {
			callback(Object.keys(errors).length === 0 ? null : createValidationError(errors), entityObject);
		});
	}
